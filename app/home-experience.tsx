...
import { GAME_LANDING_PATHS } from "@/lib/seo";

const games = GAME_CATALOG;
const plans = HOSTING_PLANS;
const region = HOSTING_REGIONS[0];
...
export function HomeExperience() {
  ...
  const [game, setGame] = useState<ActiveGameId>(DEFAULT_SERVER_DRAFT.gameId);
  const [planId, setPlanId] = useState(DEFAULT_SERVER_DRAFT.planId);
  const [backup, setBackup] = useState(DEFAULT_SERVER_DRAFT.backups);
  ...
  // Ana sayfa, yapılandırıcı ve panel aynı katalog fonksiyonunu kullanır.
  const total = calculateMonthlyPrice({ planId, regionId: region.id, backups: backup });
  ...
  const configurePlan = (selectedGame: ActiveGameId = game) => {
    const nextDraft = {
      ...DEFAULT_SERVER_DRAFT,
      gameId: selectedGame,
      softwareId: getGame(selectedGame).software[0]?.id ?? "",
      planId,
      regionId: region.id,
      backups: backup,
    };
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setCheckedEmail(betaEmail.trim());
    setSent(true);
  };
  ...
  return (
    <main>
        ...
        <div className="builderShell">
          ...
          <div className="builderCard">
            ...
            <Field label="RAM seçimi" note={plan.players}>
              <div className="ramChoices">{plans.map((item) => <button className={planId === item.id ? "selected" : ""} key={item.id} onClick={() => setPlanId(item.id)} type="button"><b>{item.ram} GB</b><small>{item.label}</small></button>)}</div>
            </Field>
            <div className="splitFields">
              <Field label="Lokasyon"><button className="selectCard" disabled type="button"><span>{region.flag}</span><b>{region.name} · {region.location}</b><em>✓</em></button></Field>
              <Field label="Günlük yedek"><button aria-checked={backup} className={"toggleCard " + (backup ? "on" : "")} onClick={() => setBackup(!backup)} role="switch" type="button"><span><b>{backup ? "Aktif" : "Kapalı"}</b><small>+{formatTry(BACKUP_MONTHLY_PRICE)} / ay</small></span><i><b /></i></button></Field>
            </div>
            <div className="price"><span><small>TAHMİNİ AYLIK</small><b>{formatTry(total)} <em>/ ay</em></b><i>KDV dahil beta fiyatı</i></span><button className="button large" onClick={() => configurePlan()} type="button">Tam yapılandırıcı <Icon name="arrow" /></button></div>
          </div>
        </div>
      </section>
      ...
      <section className="cta"><span className="brandIcon big"><i /></span><div><Kicker icon="spark">Erken erişim</Kicker><h2>İlk oyun sunucunu birlikte açalım.</h2><p>Minecraft veya Terraria beta planını hazırla; gerçek kurulum açıldığında güncel kapsamı ve nihai fiyatı önce gör.</p></div><button className="button large" onClick={reserve} type="button">Beta planını hazırla <Icon name="arrow" /></button></section>
      ...
      <footer>
        ...
        <div className="copyright"><span>© 2026 Riftory · Çalışma markası</span><span><i /> Ürün ön izlemesi yayında</span></div>
      </footer>
      ...
      {modal && (
        <div className="modalBg" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(false); }}>
          <section aria-labelledby="modal-title" aria-modal="true" className="modal" role="dialog">
            ...
            {!sent ? <>
              <div className="modalIcon"><Icon name="rocket" /></div><Kicker icon="spark">Kapalı beta</Kicker><h2 id="modal-title">Planını gözden geçir.</h2><p>Ödeme almıyoruz ve beta kaydı henüz açık değil. Kayıt altyapısı bağlandığında seçtiğin planın nihai fiyatını e-posta ile paylaşacağız.</p>
              <div className="modalPlan"><span className={"gameBadge " + (game === "terraria" ? "blue" : "")}>{game === "minecraft" ? "M" : "T"}</span><span><small>SEÇİLEN PLAN</small><b>{getGame(game).name} · {plan.ram} GB</b><i>{region.name} · {backup ? "Günlük yedek" : "Yedeksiz"}</i></span><strong>{formatTry(total)}<small>/ay tahmini</small></strong></div>
              <form onSubmit={submit}><label htmlFor="email">E-posta adresin</label><input autoFocus id="email" name="email" onChange={(event) => setBetaEmail(event.target.value)} placeholder="ornek@email.com" required type="email" value={betaEmail} /><label className="consent"><input required type="checkbox" /><span>Beta kaydı açıldığında e-posta ile bilgilendirilmeyi kabul ediyorum.</span></label><button className="button large full" type="submit">Formu doğrula <Icon name="arrow" /></button></form>
              <p className="authDisclosure"><Icon name="shield" size={14} /> Bu ön izleme formu yalnızca tarayıcında doğrulanır; e-posta göndermez ve adresini kaydetmez.</p>
            </> : <div className="success"><span><Icon name="check" size={30} /></span><Kicker icon="spark">Form doğrulandı</Kicker><h2 id="modal-title">Kayıt altyapısı bekleniyor.</h2><p><b>{checkedEmail}</b> kaydedilmedi ve bu adrese e-posta gönderilmedi. Beta kayıt listesi, e-posta sağlayıcısı bağlandığında etkinleşecek.</p><button className="ghost bordered" onClick={() => setModal(false)} type="button">Siteye dön</button></div>}
          </section>
        </div>
      )}
    </main>
...
function Dashboard() {
  return <div className="dashboard"><aside><b>R</b><span><i className="active"/><i/><i/><i/><i/></span><em>F</em></aside><div className="dashBody"><header><span><small>SUNUCULARIM / MINECRAFT · ÖN İZLEME</small><b>Fatih&apos;in Dünyası</b></span><em><i /> Aktif</em></header><div className="dashStats"><span><small>OYUNCULAR</small><b>7 <i>/ 15</i></b></span><span><small>BÖLGE</small><b>EU <i>West</i></b></span><span><small>SON YEDEK</small><b>2 <i>dk</i></b></span></div><div className="chart"><header><span><small>KAYNAK KULLANIMI</small><b>Son 12 saat</b></span><em>● RAM　● CPU</em></header><div><i/><i/><i/><i/><i/><svg aria-hidden="true" preserveAspectRatio="none" viewBox="0 0 600 130"><path d="M0 105 C45 98 55 61 105 73 S175 101 220 69 S290 38 325 60 S390 88 430 48 S515 30 600 57"/><path d="M0 87 C70 75 90 92 140 82 S220 53 265 76 S350 103 390 79 S470 63 520 76 S570 81 600 70"/></svg></div></div><div className="dashBottom"><span><small>OTOMATİK YEDEKLEME</small><b>Her gün, 04:00</b></span><button type="button">Yedek al <Icon name="arrow" size={13} /></button></div></div></div>;
}
